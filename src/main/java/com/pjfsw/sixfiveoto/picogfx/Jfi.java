...
public class Jfi {
    ...
    public Jfi(int[] data, int width, int height, int[] palette) {
        this.data = data;
        this.width = width;
        this.height = height;
    ...
    public static Jfi load(String filename) throws URISyntaxException, IOException {
        Path path = new File(PicoGfx.class.getResource(filename).toURI()).toPath();
        byte[] bytes = Files.readAllBytes(path);
        if (bytes[0] != 0x4a) { // 'J'
            throw new IllegalArgumentException(String.format("%s is not a valid JFI file!", filename));
        }
        ...
        return new Jfi(data, width, height, palette);
    }
